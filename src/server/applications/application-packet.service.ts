...
import { ApplicationPacket, type ApplicationPacketDocumentKind } from "@/server/models/ApplicationPacket";
...
function assertId(id: string) {
  if (!mongoose.isValidObjectId(id)) throw new AppError("APPLICATION_NOT_FOUND", 404, "Application not found.");
}

export async function getApplicationReadiness(userId: string, applicationId: string) {
  assertId(applicationId);
  ...
  const requiredDocumentKinds: ApplicationPacketDocumentKind[] = packet?.requiredDocumentKinds?.length
    ? [...packet.requiredDocumentKinds]
    : ["CV"];
  const recommendationsRequired = Number(packet?.recommendationsRequired ?? 0);

  const [documents, recommendations] = await Promise.all([
    StudentDocument.find({ userId, kind: { $in: requiredDocumentKinds } })
      .select("kind originalName createdAt")
      .sort({ createdAt: -1 })
      .lean(),
    RecommendationRequest.find({
      userId,
      status: "RECEIVED",
      $or: [{ studentReferenceId: application._id }, { studentReferenceId: null, applicationName: application.title }]
    })
      .select("refereeName receivedAt")
      .sort({ receivedAt: -1 })
      .lean()
  ]);
  ...
  for (const doc of documents) {
    ...
    if (!latestByKind.has(kind)) latestByKind.set(kind, { id: String(doc._id), name: String(doc.originalName) });
  }

  const checklist: Array<{
    id: string;
    category: "DETAIL" | "DOCUMENT" | "RECOMMENDATION";
    label: string;
    complete: boolean;
    href: string;
  }> = [
    ...
    ...requiredDocumentKinds.map((kind) => ({
      ...
      label: `${kind.replaceAll("_", " ")} uploaded`,
      complete: latestByKind.has(kind),
      href: "/dashboard/documents"
    })),
    ...Array.from({ length: recommendationsRequired }, (_, index) => ({
      id: `recommendation:${index + 1}`,
      category: "RECOMMENDATION" as const,
      label: `Recommendation ${index + 1} received`,
      complete: index < recommendations.length,
  ...
  const score = totalCount ? Math.round((completeCount / totalCount) * 100) : 100;
  return {
    score,
    ready: completeCount === totalCount,
    completeCount,
    totalCount,
    ...
    receivedRecommendations: recommendations.length,
    documents: Object.fromEntries(latestByKind)
  };
}
...
export async function updateApplicationPacket(userId: string, applicationId: string, input: ApplicationPacketInput) {
  ...
  await ApplicationPacket.findOneAndUpdate(
    { userId, applicationId },
    {
      $set: {
        requiredDocumentKinds: [...new Set(input.requiredDocumentKinds)],
        recommendationsRequired: input.recommendationsRequired
      },
      $setOnInsert: { userId, applicationId }
    },
