...
export default async function UniversityDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { university, professors } = await loadUniversity(slug);
    ...
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          <div><p className="text-sm font-medium text-slate-500">{[university.city, university.region, university.country].filter(Boolean).join(", ")}</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-slate-950">{university.name}</h1></div>
          <div className="flex flex-wrap items-center gap-2"><span className="w-fit rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">Source: {university.source}</span><SaveButton itemType="UNIVERSITY" targetId={university.id} /></div>
        </div>
      ...
      <section className="mt-7">
        <div className="flex items-end justify-between"><div><p className="text-sm font-medium text-slate-500">Published faculty records</p><h2 className="mt-1 text-xl font-semibold text-slate-950">Professors at this university</h2></div><span className="text-sm text-slate-500">{professors.total} records</span></div>
