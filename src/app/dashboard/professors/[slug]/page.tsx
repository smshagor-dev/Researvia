...
export default async function ProfessorDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm font-medium text-slate-500">{professor.title || professor.department || "Academic profile"}</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-slate-950">{professor.fullName}</h1><p className="mt-2 text-sm text-slate-600">{professor.universityName}{professor.country ? ` • ${professor.country}` : ""}</p></div><div className="flex flex-wrap items-center gap-2"><span className="w-fit rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">Source: {professor.source}</span><SaveButton itemType="PROFESSOR" targetId={professor.id} /></div></div>
