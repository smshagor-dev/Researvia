...
export function OutreachCreateForm({ accounts, professor }: { accounts: Account[]; professor: ProfessorTarget }) {
  ...
  if (!accounts.length) return <Alert tone="info">Connect Gmail or Microsoft before creating an outreach campaign.</Alert>;
  ...
  return <form onSubmit={submit} className="space-y-5 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
    ...
    <div className="grid gap-4 md:grid-cols-2"><div className="space-y-2"><Label htmlFor="accountId">Send from</Label><select id="accountId" name="accountId" required className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm">{accounts.map((account) => <option key={account.id} value={account.id}>{account.email} · {account.provider}</option>)}</select></div><div className="space-y-2"><Label htmlFor="name">Campaign name</Label><Input id="name" name="name" required maxLength={180} placeholder="PhD outreach · Fall intake" /></div></div>
