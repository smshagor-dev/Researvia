...
const columns = [
  ...
  { title: 'Resources', links: [{ label: 'Blog', href: '#resources' }, { label: 'Guides', href: '#resources' }, { label: 'Help Center', href: 'mailto:support@researvia.com?subject=ResearVia%20Help%20Center' }, { label: 'Webinars', href: '#resources' }, { label: 'Contact Us', href: 'mailto:support@researvia.com' }] },
  { title: 'Company', links: [{ label: 'About Us', href: '#about' }, { label: 'Careers', href: 'mailto:careers@researvia.com' }, { label: 'Privacy Policy', href: '#privacy' }, { label: 'Terms of Service', href: '#terms' }, { label: 'Cookie Policy', href: '#cookies' }] },
];
    ...
    <footer className="bg-slate-950 px-4 py-16 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-10 lg:grid-cols-[1.1fr_1.7fr_1.2fr]">
          <div>
            <Link href="/" className="flex items-center gap-2" aria-label="ResearVia home">
              ...
              <span className="text-lg font-bold tracking-tight">ResearVia</span>
            </Link>
            ...
            <div className="mt-6 flex gap-3">
              {[{ label: 'Twitter', icon: Twitter }, { label: 'LinkedIn', icon: Linkedin }, { label: 'Facebook', icon: Facebook }].map(({ label, icon: Icon }) => (
                <a key={label} href={`mailto:social@researvia.com?subject=${label}`} className="flex h-9 w-9 items-center justify-center rounded-md border border-white/10 text-slate-400 transition hover:border-indigo-400 hover:text-white" aria-label={label}>
          ...
          <div>
            ...
            <form className="mt-5 flex gap-2" action="mailto:newsletter@researvia.com" method="post">
        ...
        <div id="privacy" className="mt-12 border-t border-white/10 pt-6 text-sm text-slate-500">
          <div id="terms" />
          <div id="cookies" />
          <p>Copyright 2026 ResearVia. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
